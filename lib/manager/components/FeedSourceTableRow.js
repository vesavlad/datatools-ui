...
import type { Feed, FeedSourceSummary, ValidationSummary, Project } from '../../types'
import type {
...
type Props = ContainerProps & {
  ...
  feedSource: Feed | FeedSourceSummary,
...
export default class FeedSourceTableRow extends PureComponent<Props> {
  ...
  getVersionDisplayData (
    ...
    if (validationEndDate && validationErrorCount && validationStartDate) {
      const errorCount = validationErrorCount || this.messages('none')
      const endMoment = moment(validationEndDate)
      const startMoment = moment(validationStartDate)
  ...
  generateFeedSourceLabelRenderVars (feedSource: Feed, comparisonValidationSummary: ?ValidationSummary) {
    const {latestFeedVersionEndDate, latestFeedVersionIssues, latestFeedVersionStartDate} = feedSource
    ...
    const comparisonData = this.getVersionDisplayData(latestFeedVersionEndDate, latestFeedVersionIssues, latestFeedVersionStartDate)
  ...
  render () {
    ...
    const currentVersionData = this.getVersionDisplayData(latestFeedVersionEndDate, latestFeedVersionIssues, latestFeedVersionStartDate)
